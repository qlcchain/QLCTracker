...
async function run() {
	...
	if (isDev) {
		...
		if (platform == 'win32') {
			configTemp = path.join(global.resourcesPath, '../../../../extra/testnet_config.json');
			...
			configTemp = path.join(appPath, 'extra/testnet_config.json');
		}
	} else {
		console.log('Running in production');
		configTemp = path.join(global.resourcesPath, 'testnet_config.json');
	}
	console.log(`path: ${global.resourcesPath}, config template ${configTemp}`);
	console.log(`path: ` + toExecutableName('gqlct'));

	const cmd = path.join(global.resourcesPath, toExecutableName('gqlct'));
	const userData = app.getPath('userData');
	const configDir = path.join(userData, isDev ? 'gqlc_testnet_dev' : 'gqlc_testnet');
	...
	const config = path.join(configDir, 'qlc_wallet_testnet.json');
		...
		const template = [
			...
			},
			{
				...
				submenu: [
					...
					{
						type: 'normal',
						label: `sms-billing Version: ${autoUpdater.currentVersion}`
					},
					{
